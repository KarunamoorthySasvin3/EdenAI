...
export function ClimateChangeAnalysis({
  className,
}: ClimateChangeAnalysisProps) {
  ...
  const currentScenario =
    scenarioData[selectedScenario as keyof typeof scenarioData];

  // Adjust values based on timeframe
  const timeframeMultiplier =
    timeframe === "2030" ? 0.5 : timeframe === "2050" ? 1.0 : 2.0;
  ...
  return (
    <Card className={className}>
      <CardHeader>
        ...
        <CardDescription>
          PyTorch-powered projection of climate scenarios and your garden&apos;s
          mitigating effect
        </CardDescription>
      </CardHeader>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1 space-y-2">
            ...
            >
              ...
              <SelectContent>
                {/* Make sure each SelectItem has a non-empty string value */}
          ...
          <div className="flex-1 space-y-2">
            ...
            <Select value={timeframe} onValueChange={setTimeframe}>
              ...
              <SelectContent>
                {timeframes.map((tf) => (
                  <SelectItem
                    key={tf.id || `key-${tf.name}`}
                    value={tf.id ? tf.id : `timeframe-${tf.name}`} // Ensure value is never empty or undefined
                  >
                    {tf.name}
                  </SelectItem>
                ))}
        ...
        <Tabs defaultValue="projections">
          ...
          <TabsContent value="projections" className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="border rounded-lg p-4 space-y-2">
                ...
                <div className="text-2xl font-bold">
                  +
                  {(currentScenario.temperature * timeframeMultiplier).toFixed(
                    1
                  )}
                  °C
                </div>
                <Progress
                  value={currentScenario.temperature * timeframeMultiplier * 15}
              ...
              <div className="border rounded-lg p-4 space-y-2">
                ...
                <div className="text-2xl font-bold">
                  +
                  {Math.round(
                    currentScenario.extremeWeatherIncrease * timeframeMultiplier
                  )}
                  %
                </div>
                <Progress
                  value={
                    (currentScenario.extremeWeatherIncrease *
                      timeframeMultiplier) /
                    2
                  }
              ...
              <div className="border rounded-lg p-4 space-y-2">
                ...
                <div className="text-2xl font-bold">
                  {currentScenario.precipitationChange > 0 ? "+" : ""}
                  {Math.round(
                    currentScenario.precipitationChange * timeframeMultiplier
                  )}
                  %
                </div>
                <Progress
                  value={
                    50 +
                    currentScenario.precipitationChange *
                      timeframeMultiplier *
                      3
                  }
              ...
              <div className="border rounded-lg p-4 space-y-2">
                ...
                <div className="text-2xl font-bold">
                  {(currentScenario.seaLevelRise * timeframeMultiplier).toFixed(
                    1
                  )}
                  m
                </div>
                <Progress
                  value={
                    currentScenario.seaLevelRise * timeframeMultiplier * 60
                  }
            ...
            <div className="border rounded-lg p-4 bg-slate-50">
              ...
              <p className="text-sm text-slate-700">
                These projections are generated using an ensemble of 5 neural
                networks trained on CMIP6 climate model data. The uncertainty
                range for this {selectedRegion}
                projection is ±
                {Math.round(currentScenario.temperature * 0.2 * 10) / 10}°C for
                temperature and ±
                {Math.round(currentScenario.precipitationChange * 0.3)}% for
                precipitation.
              </p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="garden-impact" className="space-y-4">
            ...
            <div className="p-4 border rounded-lg bg-green-50">
              ...
              <p className="text-sm">
                If 20% of properties in your neighborhood implemented similar
                gardens, it could reduce local temperatures by{" "}
                {(gardenImpact.temperatureReduction * 12).toFixed(1)}°C during
                heat waves and improve stormwater management by up to 35%.
              </p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="adaptation" className="space-y-4">
            <div className="border rounded-lg p-4 space-y-3">
              <h3 className="font-medium">
                AI-Recommended Adaptation Strategies
              </h3>
              <ul className="space-y-2">
                <li className="flex items-start gap-2 text-sm">
                  ...
                  <span>
                    <strong>Drought Resilience:</strong> Increase plant
                    diversity with drought-tolerant species that can withstand{" "}
                    {Math.round(
                      currentScenario.temperature * timeframeMultiplier * 3
                    )}{" "}
                    additional drought days per year.
                  </span>
                </li>
                <li className="flex items-start gap-2 text-sm">
                  <div className="rounded-full bg-green-100 p-1 mt-0.5">
                  ...
                  <span>
                    <strong>Water Management:</strong> Implement rainwater
                    harvesting to offset the{" "}
                    {currentScenario.precipitationChange > 0
                      ? "increased intensity"
                      : "reduced frequency"}{" "}
                    of rainfall events.
                ...
                <li className="flex items-start gap-2 text-sm">
                  ...
                  <span>
                    <strong>Thermal Protection:</strong> Add shade trees on the
                    south and west sides to reduce cooling needs during
                ...
                <li className="flex items-start gap-2 text-sm">
                  ...
                  <span>
                    <strong>Biodiversity Support:</strong> Plant native species
                    that support pollinators and wildlife facing habitat
                    pressures.
                  </span>
                </li>
              </ul>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="border rounded-lg p-4">
                ...
                <p className="text-sm">
                  For your region, consider plants that will thrive in
                  conditions
                  {currentScenario.temperature > 2
                    ? " 1-2 USDA zones warmer"
                    : " with greater temperature extremes"}
                  by {timeframe}.
                </p>
              </div>
              ...
              <div className="border rounded-lg p-4">
                ...
                <p className="text-sm">
                  ...
                  capacity by {Math.round(5 + currentScenario.temperature * 10)}
      ...
      <CardFooter className="border-t pt-4">
        <div className="text-xs text-muted-foreground w-full text-center">
          Model: Climate CMIP6 Ensemble v2.1 | PyTorch Geometric Processing |
